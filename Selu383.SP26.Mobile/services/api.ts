const API_BASE_URL = 'https://cg6xt47n-7116.use2.devtunnels.ms';
...
export const login = async (username: string, password: string): Promise<any> => {
  try {
    console.log('API: Attempting login to:', API_BASE_URL + '/api/authentication/login');
...
export const getMenuItems = async (): Promise<MenuItemDto[]> => {
  try {
    console.log('API: Fetching menu items from:', `${API_BASE_URL}/api/menu/items`);
...
export const getMenuCategories = async (): Promise<MenuCategoryDto[]> => {
  try {
    console.log('API: Fetching menu categories from:', `${API_BASE_URL}/api/menu/categories`);
...
export const getMyOrders = async (): Promise<OrderDto[]> => {
  try {
    console.log('API: Fetching user orders from:', `${API_BASE_URL}/api/orders/my-orders`);
...
export const cancelReservation = async (reservationId: number): Promise<any> => {
  try {
    ...
    const response = await apiCall(`/api/reservations/${reservationId}`, 'DELETE');
